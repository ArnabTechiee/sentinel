...
<style>
  ...
  .stApp { background: #fbfbf9; }

  /* Belt and braces: the theme is pinned in .streamlit/config.toml, but pin the
     text colours here too. If config.toml is missing -- someone runs app.py
     from another directory, or copies just this file -- a dark client theme
     would render dark text on the dark-agnostic background and the metric row
     would vanish. A demo should not be one stray file away from illegible. */
  ...
  [data-testid="stSidebar"] { background: #f2f1ed; border-right: 1px solid var(--rule); }
  [data-testid="stSidebar"] * { color: var(--ink); }
  [data-testid="stSidebar"] [data-testid="stCaptionContainer"] p { color: var(--ink-soft) !important; }
...
if open_cases.empty:
    ...
    if b1.button("Block", type="primary", disabled=degraded, use_container_width=True):
        save_decision(row["transaction_id"], "BLOCK", degraded)
        st.rerun()
    if b2.button("Approve", use_container_width=True):
...
with tab_open:
    if len(open_cases) > 1:
        rest = open_cases.iloc[1:][["transaction_id", "p_shown", "amount",
                                    "expected_loss"]].copy()
        rest.columns = ["Transaction", "Score", "Amount", "Expected loss"]
...
with tab_log:
    if decisions:
        log = pd.DataFrame([
            {"Transaction": k, "Action": v["action"],
             "Scored by": v["scored_by"], "At": v["at"]}
